...
int main () {
    ...
    for (int i=0; i < x_f.size(); ++i) 
    {
        ...
        N = std::abs(x_f[i] - x_t[i]);
        M = std::abs(y_f[i] - y_t[i]);
        isDiagonal = (std::abs(x_f[i] - x_t[i]) == std::abs(y_f[i] - y_t[i]));
        
        if ( isDiagonal ) {
            for ( int k=0, l=0; k<=N, l<=M; ++k, ++l ) {
                if ( (x_f[i]<x_t[i]) && (y_f[i]<y_t[i]) ) {
                    x = x_f[i] + k;
                    y = y_f[i] + l;
                    v[y][x] += 1; 
                }
                else if ( (x_f[i]>x_t[i]) && (y_f[i]<y_t[i]) ) 
                {
                    x = x_f[i] - k;
                    y = y_f[i] + l;
                    v[y][x] += 1; 
